...
class HPTClone:
    ...
    defaults to 1). Fires its own beam volley directly from its own corner
    position — see combat.tick_hpt_clones for the synchronized volley.
    Lives and dies entirely within its owner's own SideState — never
    crosses the Solo/Battle one-tick information boundary.

    Previously fired from a separate "attack orb" offset hover_radius px
    away at a random angle — that offset point landed off-screen roughly
    70% of the time (confirmed by simulation) and, worse, rendered as a
    SECOND visible sphere distinct from the clone's own marker, which read
    as "the corner sphere isn't firing" even when its offset orb genuinely
    was. Firing straight from the clone's own on-screen corner position
    removes both problems: one visible sphere per threshold, and it's
    always the one that fires. Identical in Solo & Battle."""
    __slots__ = ("x", "y", "hp", "damage", "rgb", "cfg")
...
def tick_hpt_clones(world):
    ...
    if fire_now:
        ...
        for clone in world.clones:
            tx, ty = min(
                targets,
                key=lambda f: (f[0] - clone.x) ** 2
                            + (f[1] - clone.y) ** 2)
            ddx, ddy = tx - clone.x, ty - clone.y
            dist = math.hypot(ddx, ddy) or 1.0
            vx, vy = ddx / dist * step, ddy / dist * step
            if beam_layer is not None:
                pr = RichBeamProjectile(clone.x, clone.y, vx, vy,
                                        clone.rgb, config.BEAM_TRAIL_LEN,
                                        beam_layer)
            else:
                pr = Projectile(clone.x, clone.y, vx, vy,
                                 clone.rgb, config.BEAM_TRAIL_LEN)
                pr.style = "beam"
            pr.max_age = max_age
